...
async function insertQuestTemplate(
    quest: QuestTemplate
): Promise<boolean> {
...
async function insertAllQuestTemplates(): Promise<void> {
    let questsInserted = 0;

    for (const [i, quest] of quests.entries()) {
        const validRequiredFields =
            quest.required_fields.length > 0 &&
            !(
                quest.required_fields.length === 1 &&
                quest.required_fields[0] === ""
            );
        ...
        if (
            ...
            quest.quest_hook !== "" &&
            validRequiredFields &&
            quest.default_difficulty !== ""
        ) {
            const success = await insertQuestTemplate(
                quest
            );
...
insertAllQuestTemplates().catch(
    (error: unknown) => {
        console.error(
            "Failed to insert quest templates:",
            error
        );
    }
);
...
// node app/db_updates/insertQuestTemplates.js